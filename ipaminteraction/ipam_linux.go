...
func ConfigureIface(ifName string, res *current.Result) error {
	...
	if len(res.Interfaces) == 0 {
		logging.Debugf("IPAM ConfigureIface: no interfaces to configure")
		return fmt.Errorf("no interfaces to configure")
	}

	link, err := netlink.LinkByName(ifName)
	if err != nil {
		logging.Debugf("IPAM ConfigureIface: failed to lookup interface: %v", ifName)
		return fmt.Errorf("failed to lookup %q: %v", ifName, err)
	}

	if err := netlink.LinkSetUp(link); err != nil {
		logging.Debugf("IPAM ConfigureIface: failed to set up interface: %v", ifName)
	...
	for _, ipc := range res.IPs {
		...
		logging.Debugf("IPAM ConfigureIface: interface Index: %v", intIdx)
